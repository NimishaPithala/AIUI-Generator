...
const EXAMPLES = [
  "Interactive map of India with all states — hover for name, click for details",
  "Explain the human digestive system with an interactive diagram",
  "All Indian states — searchable cards with capital, population, area, language",
  "Interactive Solar System — all 8 planets with facts and comparisons",
  "Periodic table — first 20 elements, clickable with full details",
  "Human heart diagram — label each chamber, hover to highlight",
  "World War II key events timeline — 1939 to 1945",
  "3D Shape calculator — volume and surface area with live inputs",
];
...
export default function App() {
  const [prompt,        setPrompt]        = useState("");
  const [generatedCode, setGeneratedCode] = useState("");
  const [outputType,    setOutputType]    = useState("jsx");
  const [loading,       setLoading]       = useState(false);
  const [error,         setError]         = useState("");
  const [showCode,      setShowCode]      = useState(false);
  const [dots,          setDots]          = useState("");
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setOutputType(data.output_type || "jsx");
        setGeneratedCode(data.generated_code || "");
        setTimeout(
          () => resultRef.current?.scrollIntoView({ behavior: "smooth" }),
          200
      ...
      setError(
        e.code === "ECONNABORTED"
          ? "Request timed out — the backend may be waking up on Render free tier. Wait 30 seconds and try again."
  ...
  return (
    ...
    >
      ...
      <header style={{ textAlign: "center", padding: "3rem 1.5rem 0" }}>
        ...
        >
          ⚡ DeepSeek R1 · NVIDIA · SVG-powered
        </div>
      ...
      <main style={{ maxWidth: "840px", margin: "0 auto", padding: "0 1.25rem" }}>
        ...
        >
          ...
          <textarea
            ...
            placeholder="e.g. Show me an interactive map of India with all states…"
            style={{
              width: "100%", background: "rgba(0,0,0,.35)",
              border: "1px solid rgba(255,255,255,.15)",
              borderRadius: "12px", padding: "0.9rem 1.1rem",
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                ...
                <span
                  style={{
                    ...
                    borderRadius: "999px",
                    background: outputType === "html"
                      ? "rgba(251,146,60,.15)" : "rgba(99,102,241,.15)",
                    border: outputType === "html"
                      ? "1px solid rgba(251,146,60,.3)" : "1px solid rgba(99,102,241,.3)",
                    color: outputType === "html" ? "#fdba74" : "#a5b4fc",
                  }}
                >
            ...
            {showCode && (
              <div
                style={{
                  ...
                  borderRadius: "12px", padding: "1.1rem 1.3rem",
                  marginBottom: "1rem", fontSize: "0.75rem",
                  color: "#6ee7b7", overflow: "auto", maxHeight: "320px",
                  fontFamily: "'Fira Code',monospace", whiteSpace: "pre-wrap",
                }}
              >
                {generatedCode}
