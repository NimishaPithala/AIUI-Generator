...
const EXAMPLES = [
  ...
  "Explain the human digestive system with a diagram",
  "Show all Indian states with capital, population, area and language",
  "Interactive Solar System — all 8 planets with facts",
  "Periodic table — first 20 elements, clickable with details",
...
export default function App() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setError(
        e.code === "ECONNABORTED"
          ? "Request timed out — backend may be waking up on Render free tier. Try again in 30s."
          : `Could not reach backend: ${e.message}`
      );
    }
    setLoading(false);
  ...
  return (
    <div style={{
      ...
      <header style={{textAlign:"center",padding:"3rem 1.5rem 0"}}>
        ...
        <h1 style={{
          fontSize:"clamp(1.9rem,5vw,3rem)", fontWeight:900, margin:"0 0 0.5rem",
          background:"linear-gradient(90deg,#a78bfa,#60a5fa,#34d399)",
          WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent",
        }}>AI UI Generator</h1>
        <p style={{
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:"0.4rem",marginBottom:"1.2rem"}}>
            {EXAMPLES.map(ex => (
              <button key={ex} onClick={() => setPrompt(ex)} style={{
                ...
                border:"1px solid rgba(139,92,246,.3)",
                borderRadius:"999px", padding:"0.25rem 0.75rem",
                fontSize:"0.72rem", color:"#c4b5fd", cursor:"pointer", whiteSpace:"nowrap",
              }}>{ex}</button>
            ))}
          </div>
          ...
          <div ref={resultRef} style={{marginTop:"2.5rem"}}>
            ...
            <div style={{display:"flex",gap:"0.6rem",flexWrap:"wrap",marginBottom:"1.25rem"}}>
              {instruction && (
                ...
                }}>{showPlan ? "▼" : "▶"} Planner instruction</button>
              )}
              ...
              }}>{showCode ? "▼" : "▶"} Generated code</button>
            </div>
      ...
      <style>{`
        ...
        ::-webkit-scrollbar-thumb {
          background:rgba(139,92,246,.45); border-radius:3px;
        }
      `}</style>
    </div>
  );
